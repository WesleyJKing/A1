...
# def optimise_naive_bayes(X_train, y_train, X_test, y_test):
...
#         'fitted_model': best_nb_clf
#     }
    
#     results_df = pd.DataFrame([{
#         'alpha': results['best_alpha'],
#         'train_accuracy': accuracy_score(np.argmax(y_train, axis=1), best_nb_clf.predict(X_train)),
#         'test_accuracy': results['accuracy']
#     }])
    
#     return results, results_df, best_nb_clf

# opt = optimise_naive_bayes(X_test=X_test,X_train=X_train,y_test=y_test, y_train=y_train)
...
Now train the naive bayes model with embeddings.
...
def naive_bayes(X_train, y_train, X_test, y_test):
    ...
    # using cross-validation with 5 folds and optimizing for accuracy
